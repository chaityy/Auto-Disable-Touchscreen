"""
enable_touchscreen.py

Finds your touchscreen device automatically and enables it.
...
def find_touchscreen_ids() -> list[str]:
    """
    Look for touchscreen devices — including disabled ones.
    The -Status parameter is omitted so disabled devices still show up.
    """
    script = r"""
...
def main() -> int:
    ...
    log("enable_touchscreen.py started.")
    ...
    if not ids:
        msg = (
            ...
            "you can enable it manually there.\n\n"
            "Device Manager path: Human Interface Devices > your touchscreen\n"
            "(Disabled devices show with a down-arrow icon.)"
        )
        print(msg)
    ...
    for device_id in ids:
        ...
        try:
            powershell(f"Enable-PnpDevice -InstanceId '{escaped}' -Confirm:$false")
            print(f"ENABLED: {device_id}")
            log(f"Enabled: {device_id}")
        except Exception as exc:
            print(f"ERROR enabling {device_id}: {exc}")
            log(f"ERROR enabling {device_id}: {exc}")
            errors += 1

    if errors == 0:
        print("\nTouchscreen successfully enabled.")
    else:
        print(f"\nFinished with {errors} error(s). Check touchscreen_toggle.log for details.")

    log("enable_touchscreen.py finished.")
    input("\nPress Enter to exit...")
